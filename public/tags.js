...
riot.tag2('entry', '<div class="title">{entry.title}</div> <div class="meta"> <a href="{entry.brightspace}" target="_blank">View in Brightspace</a> <a href="{entry.contentpage}" target="_blank">View Content Page</a> <div class="errors"> <span class="error" each="{name, obj in entry.errors}"><strong>{obj.count}</strong> {name}</span> </div> </div>', 'entry { display: block; padding: 8px 20px; background: white; margin: 10px 20px; cursor: pointer; } entry:hover:not(.open) { background: rgba(0, 0, 0, 0.03); } entry .title { margin-bottom: 10px; font-weight: normal; font-size: 1rem; padding: 0px 4px; } entry .meta { display: none; } entry.open { box-shadow: 0px 0px 8px rgba(0, 0, 0, 0.1); border: 1px solid #ccc; } entry.open .meta { display: block; } entry .errors { border-radius: 3px; box-shadow: 0px 0px 0px 1px rgba(0, 0, 0, 0.1) inset; background: #f5ff6a; padding: 8px 20px; margin-top: 10px; } entry .errors .error { display: inline-block; padding: 0px 20px; }', 'onclick="{open}" class="open"', function (opts) {
  var _this = this;
